...
get_current_jffs_device(){
	# 查看当前/jffs的挂载点是什么设备，如/dev/ubi0_5, /dev/sda1；有usb2jffs的时候，/dev/sda1，无usb2jffs的时候，/dev/ubi0_5，出问题未正确挂载的时候，为空
...
get_usb2jffs_status(){
	...
	local CIFS_STATUS=$(df -h|grep "/cifs2"|awk '{print $1}'|grep "/dev/ubi0")
	...
	# user has mount USB disk to /jffs, and orgin jffs mount device: /dev/ubi0? mounted on /cifs2
	return 0
}
...
exit_install(){
	...
	case $state in
		1)
			...
			echo_date "本软件中心支持机型/平台：https://github.com/koolshare/qcasoft#qcasoft"
			echo_date "退出安装！"
